...
- **App chrome**: avatars render the **neon-gradient initials fallback only** (the Clerk photo is
  ...
  base rule in `globals.css`. The **brand mark** is the neon 11×8 "space invader" in `Logo`
  (`components/layout/`); the **favicon** is derived from it — `apps/web/public/` holds `favicon.svg`
  (+ `favicon-32.png` / `apple-touch-icon.png` fallbacks, linked in `index.html`), all regenerated by
  the dependency-free `apps/web/scripts/generate-favicon.mjs` if the mark ever changes.
