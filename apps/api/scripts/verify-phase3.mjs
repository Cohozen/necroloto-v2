...
try {
    ...
    for (const l of links) {
        ...
        let expected = 0;
        if (birth && death) {
            expected = l.bet.year === deathYear(death) ? calculPointByCelebrity(birth, death) : 0;
        }
    ...
    const deadLink = created.CelebritiesOnBet.find((c) => c.celebrityId === deadCeleb.id);
    ...
    const replaced = await bets.replaceCelebrities(created.id, [deadCeleb.name.toUpperCase()]);
    ...
    ok(replaced.CelebritiesOnBet[0].points === expectedPts, 'replaced list re-scored correctly');
