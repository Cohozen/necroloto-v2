...
const CHECKS = [
  [
    'Celebrities with birth',
    `SELECT COUNT(*)::int c FROM "Celebrity" WHERE birth IS NOT NULL`,
  ],
  [
    'Celebrities with death',
    `SELECT COUNT(*)::int c FROM "Celebrity" WHERE death IS NOT NULL`,
  ],
  ['Min birth date', `SELECT MIN(birth)::text v FROM "Celebrity"`],
  ['Max death date', `SELECT MAX(death)::text v FROM "Celebrity"`],
  [
    'Sum of all points',
    `SELECT COALESCE(SUM(points),0)::text v FROM "CelebritiesOnBet"`,
  ],
  [
    'Distinct bet years',
...
try {
  ...
  for (const [label, sql] of CHECKS) {
    ...
    if (!ok) allOk = false;
    console.log(
      `  ${ok ? '✅' : '❌'} ${label.padEnd(28)} neon=${sv}  supa=${tv}`,
    );
  }
  console.log(
    allOk
