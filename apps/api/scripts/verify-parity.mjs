...
const CHECKS = [
  ...
  ['Distinct bet years', `SELECT string_agg(DISTINCT year::text, ',' ORDER BY year::text) v FROM "Bet"`],
  ['Sample celeb (by id order)', `SELECT name || '|' || COALESCE(birth::text,'-') || '|' || COALESCE(death::text,'-') v FROM "Celebrity" ORDER BY id LIMIT 1`],
  ['Membership roles', `SELECT string_agg(role::text, ',' ORDER BY role::text) v FROM "Membership"`],
];
