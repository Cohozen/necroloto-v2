...
/** Wraps a rail item in a right-anchored tooltip showing its label on hover. */
            ...
            <aside className="hidden w-[78px] shrink-0 flex-col items-center gap-2 border-r border-line bg-gradient-to-b from-bg-2 to-bg/40 py-5 md:flex">
                ...
                {NAV_ITEMS.map(({ label, to, icon: Icon }) => (
                    <RailTooltip key={to} label={label}>
                        <Link to={to} aria-label={label} className={railLink}>
                            <Icon size={22} />
                        </Link>
                    </RailTooltip>
                ))}
                ...
                {isAdmin && (
                    <>
                        <RailTooltip label="Administration">
                            <Link
